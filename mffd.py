...
def mffd(*items, **kwargs):
    ...
    while index_a < len(list_a):
        ...
        try:
            ...
            if sum((size_c1, size_c2, size_a)) > binsize:
                break # Optimisation: These are the smallest values, as this is
        ...
        if sum((size_c1, size_c2, size_a)) <= binsize:
            for index_c2_new, item_c2_new in enumerate(list_c):
                ...
                item_c2 = list_c[index_c2]

        assert item_c1 == list_c[index_c1], "index_c1 out of sync!"
        assert item_c2 == list_c[index_c2], "index_c2 out of sync!"
        assert index_c1 != index_c2, "Index collision"

        # Remove the larger index first (or it _will_ break)
        # list_c = [x for e,x in enumerate(list_c) if e not in (index_c1, index_c2)]
        ...
        list_a[index_a] = [item_a, item_c1, item_c2]
        index_a += 1
    ...
    for x in ffd(*items, key=key, binsize=binsize, bins=list_a):
        yield x
